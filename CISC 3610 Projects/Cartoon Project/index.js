...
//   Cartoon.xPos =  currentTime % Cartoon.canvas.width;
  if(Cartoon.cloud1Pos-150 > canvas.width)
    Cartoon.cloud1Pos = -150;
    if(Cartoon.cloud2Pos-150 > canvas.width)
    Cartoon.cloud2Pos = -200;

  Cartoon.cloud1Pos+= .25;
  Cartoon.cloud2Pos+= .25;
...
function drawClouds(ctx, canvas){
  ...
  ctx.beginPath();
  ctx.ellipse(Cartoon.cloud1Pos, canvas.height/8+20, 100, 20, 0,  2*Math.PI, 0);
  ctx.ellipse(Cartoon.cloud1Pos+70, canvas.height/8, 100, 20, 0, 2*Math.PI, 0);
  ctx.fill();
  ctx.closePath();
  ...
  ctx.ellipse(Cartoon.cloud2Pos+70, canvas.height/8-20, 100, 20, 0,  2*Math.PI, 0);
  ctx.fill();
  ctx.closePath();
  // ctx.beginPath();
...
function drawWater(ctx, canvas){
  ...
  var width = canvas.width;
    ...
    for (var j = 0; j < 20; j++) {
      ...
      ctx.fillRect(0, canvas.height/3.30+ 100/20*j, width, 100);
    }
...
//f5f2d0
  ...
  function drawWindows(ctx, canvas, upperx, uppery) {
    ...
    ctx.fillRect(upperx+ canvas.width/5/2-105, uppery+10, 25, 80/8*8)
    ctx.fillRect(upperx+ canvas.width/5/2-30, uppery+10, 25, 80/8*8)
    //to make lined window cover

    for (let i=0; i<=8; i++){
      ctx.strokeRect(upperx+ canvas.width/5/2-105, uppery+10, 25, 80/8*i)
      ctx.strokeRect(upperx+ canvas.width/5/2-30, uppery+10, 25, 80/8*i)
        ...
        ctx.fillRect(upperx+ canvas.width/5/2+75, uppery+10, 25, 80/8*8)
        //to make lined window cover

        for (let i=0; i<=8; i++){
          ctx.strokeRect(upperx+ canvas.width/5/2, uppery+10, 25, 80/8*i)
          ctx.strokeRect(upperx+ canvas.width/5/2+75, uppery+10, 25, 80/8*i)
  ...
  function drawDoors(ctx, canvas, upperx, uppery){
    ...
    ctx.fillStyle = "#d2b48c";
    ctx.fillRect(upperx+canvas.width/5/2-30, uppery-canvas.width/5/2+230, 60, 100);
    ctx.strokeRect(upperx+canvas.width/5/2-30, uppery-canvas.width/5/2+230, 60, 100);

    ctx.fillStyle = "darkgray"
    ctx.beginPath();
    ctx.arc(upperx+ canvas.width/5/2-20, uppery-canvas.width/5/2+290, 5, 0, Math.PI*2);
    ctx.fill();
    ctx.closePath();
  }
  ...
  function drawPainting(ctx, canvas){
    ...
    for (var i = 0; i < numSquares; i++) {
      for (var j = 0; j < numSquares; j++) {
        ctx.fillStyle = 'rgb(' + (255-i*255/numSquares)  + ',' + (255-j*255/numSquares) +','+ (255-i*255/numSquares)+')';
        ctx.fillRect(950-i*width/numSquares, 400-j*width/numSquares, width/numSquares, width/numSquares);
      }
    }
...
function drawFlowers(ctx, canvas, centerX, centerY, radius, color){
  ...
  // ctx.Wed = 2;
  ctx.moveTo(centerX-radius, centerY/.99);
  ctx.bezierCurveTo(centerX, centerY, centerX-radius*3, centerY+radius*3, centerX, centerY+radius*4);
 ctx.stroke();
 //ctx.strokeStyle = "black";
  ...
  ctx.arc(centerX-radius/2.2, centerY+radius*1.1, radius, 0, Math.PI*2);
  ctx.moveTo(centerX-radius*1.5, centerY+radius*1.1)
  ctx.arc(centerX-radius*1.5, centerY+radius*1.1, radius, 0, Math.PI*2);
 ...
 ctx.beginPath()
 ctx.fillStyle = "gold"
 ctx.arc(centerX-radius, centerY/.99, radius, 0, Math.PI*2);
 ctx.fill();
 ctx.stroke();
