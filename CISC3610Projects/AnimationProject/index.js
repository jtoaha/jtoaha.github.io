/*
Jamila Toaha
This Animation Project utilizes different spritesheets to form a scene with different characters.
-Animation loops after approximately 3-5 minutes (once klY reaches a 1000)
*/
...
function update() {
  //animation will loop after klY is more than a 1000
  if (klY > 1000) {
    setupInitialValues()
  }
  ...
  if (helloKittyCurrentFrame < helloKittyJSON.frames.length - 1)
    helloKittyCurrentFrame++
  else helloKittyCurrentFrame = 0

  if (linkRunCurrentFrame < linkRunJSON.frames.length - 3)
    linkRunCurrentFrame += 3
  ...
  if (gokuX < canvas.width / 3) gokuX++

  //adjust Goku frame
  if (catitudeCurrentFrame < catitudeJSON.frames.length - 1)
    catitudeCurrentFrame++
  ...
  if (sailorWindCurrentFrame < sailorWindJSON.frames.length - 1)
    sailorWindCurrentFrame++
  ...
  if (canvas.width + ppgWalkX <= 200) {
    if (bpFlipCurrentFrame < bpFlipJSON.frames.length - 1) bpFlipCurrentFrame++
    else {
      bpSwitch = true
...
function draw() {
  ...
  canvasContext.drawImage(
    sailorWindSpriteSheet,
    currentSailorWind.frame.x,
    currentSailorWind.frame.y,
    currentSailorWind.frame.w,
    currentSailorWind.frame.h,
    canvas.width - 400,
    10,
    currentSailorWind.frame.w / 2.25,
    currentSailorWind.frame.h / 2.25
  )
  ...
  if (canvas.width + ppgWalkX <= 200) {
    ...
    if (!bpSwitch) {
      ...
      canvasContext.drawImage(
        bpFlipSpriteSheet,
        currentbpFlip.frame.x,
        currentbpFlip.frame.y,
        currentbpFlip.frame.w,
        currentbpFlip.frame.h,
        canvas.width / 2 - 150,
        10,
        currentbpFlip.frame.w / 1.5,
        currentbpFlip.frame.h / 1.5
      )
    } else {
      ...
      canvasContext.drawImage(
        bpRunSpriteSheet,
        currentbpRun.frame.x,
        currentbpRun.frame.y,
        currentbpRun.frame.w,
        currentbpRun.frame.h,
        canvas.width / 2 - 190,
        bpRunY,
        currentbpRun.frame.w / 1.5,
        currentbpRun.frame.h / 1.5
      )
    }
  }
  ...
  canvasContext.drawImage(
    helloKittySpriteSheet,
    currentHelloKitty.frame.x,
    currentHelloKitty.frame.y,
    currentHelloKitty.frame.w,
    currentHelloKitty.frame.h,
    canvas.width / 2,
    40 + klY,
    currentHelloKitty.frame.w / klScale,
    currentHelloKitty.frame.h / klScale
  )
  ...
  canvasContext.drawImage(
    linkRunSpriteSheet,
    currentLinkRun.frame.x,
    currentLinkRun.frame.y,
    currentLinkRun.frame.w,
    currentLinkRun.frame.h,
    canvas.width / 2 - 125,
    0 + klY,
    currentLinkRun.frame.w / klScale / 1.5,
    currentLinkRun.frame.h / klScale / 1.5
  )
  ...
  canvasContext.drawImage(
    ...
    canvas.width - bb8X,
    200,
    currentbb8.frame.w / 1.5,
    currentbb8.frame.h / 1.5
  )
  ...
  canvasContext.drawImage(
    ppgWalkSpriteSheet,
    currentppgWalk.frame.x,
    currentppgWalk.frame.y,
    currentppgWalk.frame.w,
    currentppgWalk.frame.h,
    canvas.width + ppgWalkX,
    175,
    currentppgWalk.frame.w / 2,
    currentppgWalk.frame.h / 2
  )
  ...
  canvasContext.drawImage(
    ...
    0 + gokuX,
    0,
    currentGoku.frame.w / 1.5,
    currentGoku.frame.h / 1.5
  )
  ...
  canvasContext.drawImage(
    roxasSpriteSheet,
    currentRoxas.frame.x,
    currentRoxas.frame.y,
    currentRoxas.frame.w,
    currentRoxas.frame.h,
    canvas.width + roxasX,
    200,
    currentRoxas.frame.w / 1.2,
    currentRoxas.frame.h / 1.2
  )
  ...
  canvasContext.drawImage(
    catitudeSpriteSheet,
    currentCatitude.frame.x,
    currentCatitude.frame.y,
    currentCatitude.frame.w,
    currentCatitude.frame.h,
    canvas.width + catitudeX,
    400,
    currentCatitude.frame.w / 4,
    currentCatitude.frame.h / 4
  )
  ...
  canvasContext.drawImage(
    sashaSpriteSheet,
    currentSasha.frame.x,
    currentSasha.frame.y,
    currentSasha.frame.w,
    currentSasha.frame.h,
    canvas.width - 300,
    0,
    currentSasha.frame.w / 1.5,
    currentSasha.frame.h / 1.5
  )
  ...
  canvasContext.drawImage(
    ...
    0,
    310,
    currentLevi.frame.w / 1.5,
    currentLevi.frame.h / 1.5
  )
}
...
function drawSpeechLinkKitty(currentX, currentY, maxWidth, maxHeight) {
  ...
  if (klY > 28 && klY < 60) {
    ...
    wrapText(
      canvasContext,
      'Oh no! Princess Zelda is in trouble.',
      canvas.width / 3 + 10,
      klY,
      100,
      15
  ...
  if (klY > 290 && klY < 310) {
    ...
    wrapText(
      canvasContext,
      'Yes, I actually opened up a portal for him',
      currentX + 230,
      klY,
      100,
      15
  ...
  if (klY > 600 && klY < 720) {
    ...
    canvasContext.fillStyle = '#ffffff'
    canvasContext.beginPath()
    canvasContext.ellipse(300, 310, 155, 10 * 4, 0, 0, Math.PI * 2)
    canvasContext.fill()
    ...
    wrapText(
      canvasContext,
      'Hey, would it kill you to clean your screen once in a while...',
      170,
      305,
      300,
      15
